...
def main():
    ...
    retarget = Retarget_train(30000)  # 增大数据量
    retarget.retarget()
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}") as pbar:
            for inputs, targets in pbar:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    loss = F.mse_loss(outputs[6:], targets[:,10:14])
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            val_loss = F.mse_loss(val_outputs[6:], val_targets[:,10:14]).item()
    ...
    # 可视化
    print(pred_qpos.shape,retarget.joints.shape)
    retarget.qpos[:,6:]*=0
    pred_qpos= torch.cat([retarget.qpos[:,:10].cpu(),pred_qpos],dim=1)
    pred_qpos= torch.cat([pred_qpos,retarget.qpos[:,14:].cpu()],dim=1)
